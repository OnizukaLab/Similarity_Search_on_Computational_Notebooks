...
"""
(jupyter/jupyter.pyのrequest_varに複製してあり，実際はそちらのみ使用?)
Module that injects code into the kernel to allow printing variables.
print_varの引数は(カーネルID，表形式またはリストの変数名)を指定し，カーネルに接続する．
その後，引数の変数がPandasのDataFrame, NumPyのndarray, listのいずれかの場合は，
その変数の内容をjson形式に変換して出力する．
"""
