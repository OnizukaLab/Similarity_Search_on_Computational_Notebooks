...
sys.path.append(f"{current_dir}/juneau_copy")
sys.path.append(f"{current_dir}/core-detection_copy/lib")
#print(f"dir appended...{current_dir}/juneau_copy")
#print(f"dir appended...{current_dir}/core-detection_copy/lib")
...
class WorkflowMatching:
    ...
    #実際のNBの一部
    def make_sample_query_nx_with_cycle_1(self): #論文用
        # DataSet16, mobile-phone-pricing-predictions.ipynb
        self.QueryGraph=nx.DiGraph()
        ...
        self.QueryGraph.add_edge("wildcard_1", "cell_query_2")
        self.QueryGraph.add_edge("cell_query_1", "query_var1")
        self.QueryGraph.add_edge("query_var1", "cell_query_2")

        self.query_root="cell_query_1"
        self.query_table["query_var1"]=self.fetch_var_table("5_train_lephonepricingpredictions")

        code_table=self.fetch_source_code_table("lephonepricingpredictions")
        code=code_table[code_table["cell_id"]==5]["cell_code"].values
        code="".join(list(code))
        self.query_cell_code[f"cell_query_1"]=code
        code=code_table[code_table["cell_id"]==13]["cell_code"].values
        code="".join(list(code))
        self.query_cell_code[f"cell_query_2"]=code
        ...
        self.attr_of_q_display_type=nx.get_node_attributes(self.QueryGraph, "display_type")
        self.query_lib=self.fetch_all_library_from_db("lephonepricingpredictions")
        self.query_workflow_info={"Cell": 2, "Var": 1, "Display_data": {}, "max_indegree": 2, "max_outdegree": 2}
        self.set_query_workflow_info_Display_data()
    ...
    def make_query_for_node_num_11(self):
        ...
        for i in range(1,6):
            self.QueryGraph.add_edge(f"cell_query_{i}", f"cell_query_{i+1}")
        self.QueryGraph.add_edge("cell_query_1", "query_var1")
        self.QueryGraph.add_edge("cell_query_6", "query_var2")
        self.QueryGraph.add_edge("cell_query_1", "query_display1")
        self.QueryGraph.add_edge("cell_query_2", "query_display2")
        self.QueryGraph.add_edge("cell_query_3", "query_display3")
        ...
        self.attr_of_q_display_type=nx.get_node_attributes(self.QueryGraph, "display_type")
        self.query_lib=self.fetch_all_library_from_db("nerationpredictionxgboost")
        self.query_workflow_info={"Cell": 6, "Var": 2, "Display_data": {"DataFrame": 1, "text":1, "png":1}, "max_indegree": 1, "max_outdegree": 3}
        self.set_query_workflow_info_Display_data()
