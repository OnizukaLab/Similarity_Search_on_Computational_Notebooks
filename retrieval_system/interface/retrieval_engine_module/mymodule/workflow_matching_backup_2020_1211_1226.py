...
class WorkflowMatching:
    ...
    #実際のNBの一部
    def make_sample_query_nx2_4(self):
        # power-generation-prediction-xgboost.ipynbの一部
        self.QueryGraph=nx.DiGraph()
        ...
        self.QueryGraph.add_node("query_var1", node_type="Var", data_type="pandas.core.frame.DataFrame")
        #参考: self.G.add_node(node["name"], node_type="Display_data", nb_name=node["nb_name"], display_type=node["data_type"], cell_id=node["real_cell_id"])
        self.QueryGraph.add_node("query_display1", node_type="Display_data", display_type="DataFrame", cell_id="6")
        ...
        # テーブルのセット
        self.query_table["query_var1"]=self.fetch_var_table("6_plant_df_nerationpredictionxgboost")

        code_table=self.fetch_source_code_table("nerationpredictionxgboost")
        code=code_table[code_table["cell_id"]==6]["cell_code"].values
        code="".join(list(code))
        self.query_cell_code[f"cell_query_1"]=code
        code=code_table[code_table["cell_id"]==7]["cell_code"].values
        code="".join(list(code))
        self.query_cell_code[f"cell_query_2"]=code
        code=code_table[code_table["cell_id"]==8]["cell_code"].values
        code="".join(list(code))
        self.query_cell_code[f"cell_query_3"]=code

        # ライブラリのセット
        self.query_lib=self.fetch_all_library_from_db("nerationpredictionxgboost")

        self.attr_of_q_node_type = nx.get_node_attributes(self.QueryGraph, "node_type")
        self.attr_of_q_real_cell_id=nx.get_node_attributes(self.QueryGraph, "real_cell_id")
        self.attr_of_q_display_type=nx.get_node_attributes(self.QueryGraph, "display_type")
        self.query_workflow_info={"Cell": 3, "Var": 1, "Display_data": {"DataFrame": 1}, "max_indegree": 1, "max_outdegree": 3}
        self.set_query_workflow_info_Display_data()

    #実際のNBの一部
    def make_sample_query_nx2_5(self):
        # power-generation-prediction-xgboost.ipynbの一部
        self.QueryGraph=nx.DiGraph()
        ...
        # テーブルのセット
        self.query_table["query_var1"]=self.fetch_var_table("6_plant_df_nerationpredictionxgboost")
        self.query_table["query_var2"]=self.fetch_var_table("10_plant_df_nerationpredictionxgboost")
        ...
        code="".join(list(code))
        self.query_cell_code[f"cell_query_3"]=code
        code=code_table[code_table["cell_id"]==10]["cell_code"].values
        code="".join(list(code))
        self.query_cell_code[f"cell_query_4"]=code
        ...
        self.attr_of_q_real_cell_id=nx.get_node_attributes(self.QueryGraph, "real_cell_id")
        self.attr_of_q_display_type=nx.get_node_attributes(self.QueryGraph, "display_type")
        self.query_workflow_info={"Cell": 4, "Var": 2, "Display_data": {"DataFrame": 1}, "max_indegree": 1, "max_outdegree": 3}
        self.set_query_workflow_info_Display_data()

    #実際のNBの一部
    def make_sample_query_nx2_6(self):
        # comprehensive-eda-viz-linear-regression.ipynbの一部
        self.QueryGraph=nx.DiGraph()

        # 頂点のセット
        self.QueryGraph.add_node(f"cell_query_1", node_type="Cell", real_cell_id=f"1")
        self.QueryGraph.add_node("query_var1", node_type="Var", data_type="pandas.core.frame.DataFrame")
        ...
        # テーブルのセット
        self.query_table["query_var1"]=self.fetch_var_table("5_df_iveedavizlinearregression")

        code_table=self.fetch_source_code_table("iveedavizlinearregression")
        code=code_table[code_table["cell_id"]==5]["cell_code"].values
        code="".join(list(code))
        self.query_cell_code[f"cell_query_1"]=code

        # ライブラリのセット
        self.query_lib=self.fetch_all_library_from_db("iveedavizlinearregression")

        self.attr_of_q_node_type = nx.get_node_attributes(self.QueryGraph, "node_type")
        self.attr_of_q_real_cell_id=nx.get_node_attributes(self.QueryGraph, "real_cell_id")
        self.attr_of_q_display_type=nx.get_node_attributes(self.QueryGraph, "display_type")
        self.query_workflow_info={"Cell": 1, "Var": 1, "Display_data": {}, "max_indegree": 1, "max_outdegree": 1}
        self.set_query_workflow_info_Display_data()

    #実際のNBの一部
    def make_sample_query_nx2_7(self):
        # make_sample_query_nx2_3のセル1つ版
        self.QueryGraph=nx.DiGraph()
    ...
    #実際のNBの一部 
    def make_sample_query_nx3_1(self):
        self.QueryGraph=nx.DiGraph()

        # 頂点のセット
        for i in range(1,5):
            self.QueryGraph.add_node(f"cell_query_{i}", node_type="Cell", real_cell_id=f"{i}")
        self.QueryGraph.add_node("query_wildcard1", node_type="OneWildcard")

        # 辺のセット
        self.QueryGraph.add_edge("cell_query_1", "cell_query_2")
        self.QueryGraph.add_edge("cell_query_2", "query_wildcard1")
        self.QueryGraph.add_edge("query_wildcard1", "cell_query_3")
        self.QueryGraph.add_edge("cell_query_3", "cell_query_4")
        ...
        self.attr_of_q_real_cell_id=nx.get_node_attributes(self.QueryGraph, "real_cell_id")
        self.attr_of_q_display_type=nx.get_node_attributes(self.QueryGraph, "display_type")
        self.query_workflow_info={"Cell": 4, "Var": 0, "Display_data": {}, "max_indegree": 1, "max_outdegree": 2}
        self.set_query_workflow_info_Display_data()

    #実際のNBの一部 
    def make_sample_query_nx3_2(self):
        self.QueryGraph=nx.DiGraph()
        ...
        self.QueryGraph.add_edge("cell_query_2", "cell_query_3")
        self.QueryGraph.add_edge("cell_query_3", "cell_query_4")
        self.QueryGraph.add_edge("cell_query_4", "query_wildcard1")
    ...
    #実際のNBの一部 
    def make_sample_query_nx3_3(self):
        self.QueryGraph=nx.DiGraph()
        ...
        # 辺のセット
        self.QueryGraph.add_edge("query_wildcard1", "cell_query_1")
        self.QueryGraph.add_edge("cell_query_1", "cell_query_2")
        self.QueryGraph.add_edge("cell_query_2", "cell_query_3")
        self.QueryGraph.add_edge("cell_query_3", "cell_query_4")

        # 根を設定
        self.query_root="query_wildcard1"
    ...
    #実際のNBの一部 
    def make_sample_query_nx4_1(self):
        self.QueryGraph=nx.DiGraph()

        # 頂点のセット
        for i in range(1,5):
            self.QueryGraph.add_node(f"cell_query_{i}", node_type="Cell", real_cell_id=f"{i}")
        self.QueryGraph.add_node("query_anywildcard1", node_type="AnyWildcard")

        # 辺のセット
        self.QueryGraph.add_edge("cell_query_1", "cell_query_2")
        self.QueryGraph.add_edge("cell_query_2", "cell_query_3")
        self.QueryGraph.add_edge("cell_query_3", "cell_query_4")
        self.QueryGraph.add_edge("cell_query_3", "query_anywildcard1")
    ...
    #実際のNBの一部 
    def make_sample_query_nx4_2(self):
        self.QueryGraph=nx.DiGraph()
        ...
        # 辺のセット
        self.QueryGraph.add_edge("query_anywildcard1", "cell_query_1")
        self.QueryGraph.add_edge("cell_query_1", "cell_query_2")
        self.QueryGraph.add_edge("cell_query_2", "cell_query_3")
        self.QueryGraph.add_edge("cell_query_3", "cell_query_4")

        # 根を設定
        self.query_root="query_anywildcard1"
    ...
    #実際のNBの一部 
    def make_query_for_user_exp(self, workflow_type, cleaned_nb_name, cell_id_list, var_name_list, display_type_list):
        if workflow_type==1:
        ...
        for var_name in var_name_list:
            self.query_table[f"query_var{i}"]=self.fetch_var_table(f"{var_name}")
            i+=1
    ...
    #不使用
    def make_artifical_dataset(self, nb_name, node_nameA, node_nameB):
        self.ArtificalG=nx.DiGraph()
        for n in self.G:
            n_nb_name=self.attr_of_db_nb_name[n]
            if n_nb_name!=nb_name:
                continue
            node_type=self.attr_of_db_node_type[n]
            if node_type=="Var":
                self.ArtificalG.add_node(n, node_type="Var", nb_name=n_nb_name)
            elif node_type=="Cell":
                self.ArtificalG.add_node(n, node_type="Cell", nb_name=n_nb_name, real_cell_id=self.attr_of_db_real_cell_id[n])
            elif node_type=="Display_data":
                self.ArtificalG.add_node(n, node_type="Display_data", nb_name=n_nb_name, display_type=self.attr_of_db_display_type[n], cell_id=self.attr_of_db_real_cell_id[n])
        
        for e in (self.G.edges):
            nA=e[0]
            nB=e[1]
            if self.attr_of_db_nb_name[nA]!=nb_name or self.attr_of_db_nb_name[nB]!=nb_name:
                continue
            #if nA in [node_nameA, node_nameB] and nB in [node_nameA, node_nameB]:
            if (nA==node_nameA and nB==node_nameB) or (nA==node_nameB and nB==node_nameA):
                self.ArtificalG.add_edge(node_nameB, node_nameA)
            else:
    ...
    def set_query_workflow_info(self):
        ...
        for n in self.QueryGraph.nodes():
            indegree=0
            for i in self.G.predecessors(n):
                indegree+=1
            self.query_workflow_info["indegree"]=max(indegree, self.query_workflow_info["indegree"])
            
            outdegree=0
            for i in self.G.successors(n):
                outdegree+=1
            self.query_workflow_info["outdegree"]=max(outdegree, self.query_workflow_info["outdegree"])
    ...
    def subgraph_matching_old(self):
        matcher = NodeMatcher(self.graph_db) #matcherの初期化
        r_matcher = RelationshipMatcher(self.graph_db) #matcherの初期化
        cell_node_list = matcher.match("Cell").all()

        for node_db in cell_node_list:
            node_stack=[]
            node_stack.append([self.query_root, node_db])
            # dfs()
            while len(node_stack) > 0: #stackではなく再帰関数でDFSしたほうがよさそう
                node_pair = node_stack.pop()
                current_node_q = node_pair[0]
                current_node_db = node_pair[1]
                next_q_list = r_matcher.match((current_node_q, )).all()
                next_db_list = r_matcher.match((current_node_db, )).all()
                for next_q in next_q_list:
                    for next_db in next_db_list:
                        if next_q.labels == next_db.labels:
                            pass
    ...
    def set_db_graph_old(self):
        ...
        for start_node in node_list:
            ...
            for rel in rel_list:
                ...
                #    if err:
                #        continue
                self.G.add_edge(start_node["name"], end_node["name"], edge_type=type(rel))
    ...
    #先にコード，ライブラリ，出力の関連度を計算してからデータの関連度を計算
    #def new_proposal_method(self):
    #    self.subgraph_matching(tflg=True)
    #    self.calc_nb_score_for_new_proposal_method()
    #    pass
    ...
    # 不使用(real_subgraph_matching_part_with_timecountと合うように修正必要)
    def real_subgraph_matching_part_without_timecount(self, limit=None, nb_limit=None):
        """
        先に関数set_db_graphでデータベースから読み込んでグラフをself.Gに格納している必要あり．
        サブグラフマッチングを一括で行う．
        クエリグラフはself.QueryGraph
        ...
        if query_label in ["Cell", "Var"]:
            ...
            node_db_list=self.attr_of_db_node_type.keys() # list[str]
    ...
    def set_flg_running_faster(self, flg_chk_invalid_by_workflow_structure=True, flg_flg_prune_under_sim=True):
    ...
    # 不使用
    def rec_new4_weaving3_old(self, ret_list, snode_q, snode_db, visited, matched, nb_name, current_score):
        """
        self.Gを利用して一括でサブグラフマッチング．(先にデータベースから一括でロード)
        クエリグラフはself.QueryGraph
        wildcardがある場合および閉路を持つクエリでは誤ったサブグラフが検出される可能性あり．
    ...
    # 不使用
    def calc_nb_score_for_new_proposal_method_1(self, w_c=None, w_v=None, w_l=None, w_d=None):
        """
        スコア計算部分の時間計測あり
        self.ans_listの集合 --- ノード名(str)
        ...
        for nb_name in self.ans_list: # コード，ライブラリ，出力の類似度を計算
            ...
            for subgraph in self.ans_list[nb_name]:
                ...
                subgraph_id[ind]=subgraph
                subgraph_score[ind]=current_score

            if not self.flg_running_faster["flg_prune_under_sim"] or len(self.nb_score)<self.k:
                k_score=0
            else:
                self.top_k_score=sorted(self.nb_score.items(), key=lambda d: d[1], reverse=True)
                k_score=self.top_k_score[self.k-1][1]
            ...
            for pair in sorted_subgraph:
                subgraph=subgraph_id[pair[0]]
                #current_score=subgraph_score[subgraph]
                current_score = pair[1]

                if self.flg_running_faster["flg_prune_under_sim"] and self.flg_prune_under_sim_for_new_proposal_method(v_count, current_score, k_score):
                    continue
                if self.flg_running_faster["flg_prune_under_sim"] and self.flg_prune_under_sim_for_new_proposal_method(v_count, current_score, self.nb_score[nb_name]):
                    continue

                for n_tuple in subgraph:
                    n1_name=n_tuple[0]
                    n2_name=n_tuple[1]
                    current_score+=self.calc_rel_v_with_timecount(n1_name, n2_name)
    ...
    # 不使用
    def calc_nb_score_for_new_proposal_method_2_old(self, w_c=None, w_v=None, w_l=None, w_d=None):
        """
        スコア計算部分の時間計測あり
        self.ans_listの集合 --- ノード名(str)
        ...
        for nb_name in self.ans_list: # データの類似度を計算
            if len(self.nb_score)<self.k:
                k_score=0
            else:
                self.top_k_score=sorted(self.nb_score.items(), key=lambda d: d[1], reverse=True)
            ...
            for pair in sorted_subgraph:
                ...
                if self.flg_prune_under_sim_for_new_proposal_method(v_count, current_score, k_score):
                    continue
                if self.flg_prune_under_sim_for_new_proposal_method(v_count, current_score, self.nb_score[nb_name]):
                    continue
    ...
    # 不使用
    def calc_nb_score_for_new_proposal_method_2(self, w_c=None, w_v=None, w_l=None, w_d=None):
        """
        スコア計算部分の時間計測あり
        self.ans_listの集合 --- ノード名(str)
        検出グラフに対して1回だけ類似度下限値の比較を行う．
        ...
        for pair in sorted_subgraph:
            ...
            if nb_name not in self.nb_score:
                self.nb_score[nb_name]=0
            elif self.flg_running_faster["flg_prune_under_sim"] and self.flg_prune_under_sim_for_new_proposal_method(v_count, current_score, self.nb_score[nb_name]):
                continue
    ...
    # 不使用
    def calc_nb_score_for_new_proposal_method_2_2(self, w_c=None, w_v=None, w_l=None, w_d=None):
        """
        スコア計算部分の時間計測あり
        self.ans_listの集合 --- ノード名(str)
        データ関連度の重みがゼロの時はもっと高速な別の関数に切り替える．
        この関数ではデータ関連度を1つ計算するたびに類似度下限値との比較を行う．
        さらに，類似度下限値との比較の関数も一部改良している．(計算回数を減らす)
        """
        self.init_each_calc_time_sum()
        for w in [w_c, w_v, w_l, w_d]:
            if not w is None:
        ...
        for pair in sorted_subgraph:
            ...
            for n_tuple in subgraph:
                ...
                if not self.flg_running_faster["flg_prune_under_sim"]:
                    continue
                if remain_v_count==0 or self.flg_prune_under_sim_for_new_proposal_method_2(remain_v_count, current_score, [k_score, self.nb_score[nb_name]]):
                    break
    ...
    def calc_nb_score_for_new_proposal_method_2_3_wv0(self, w_c=None, w_v=None, w_l=None, w_d=None):
        ...
        for pair in sorted_subgraph:
            ...
            if self.flg_prune_under_sim_for_new_proposal_method_3(self, self.w_c, c_count, current_score, k_score):
                continue

            if nb_name not in self.nb_score:
                self.nb_score[nb_name]=0
            elif self.flg_prune_under_sim_for_new_proposal_method(c_count, current_score, self.nb_score[nb_name]):
                continue
    ...
    def existing_method_calc_code_sim_chk(self):
        """
        セルを頭から結合したソースコードだけをみてtop-kのスコアにする．(self.nb_scoreにセット)
        """
        class_code_relatedness=self.class_code_relatedness
        ...
        for nb_name in self.valid_nb_name:
            ...
            self.nb_score[nb_name]=sim
            break
        return  query_code, db_code, nb_name
            


    def existing_method_sum(self, flg_table_sim=True, flg_code_sim=True, flg_library_sim=True, flg_display_sim=True):
        self.init_each_calc_time_sum()
        ...
        if self.w_v!=0:
            for i in range(len(sorted_nb_score)):
                ...
                if len(self.nb_score)<self.k:
                    ...
                    self.top_k_score=sorted(self.nb_score.items(), key=lambda d: d[1], reverse=True)
                    k_score=self.top_k_score[self.k-1][1]
                if self.flg_prune_under_sim_for_new_proposal_method(v_count=n_q_num, current_score=current_nb_score[nb_name], compare_score=k_score):
                    continue
                self.nb_score[nb_name]=current_nb_score[nb_name]+self.existing_method_calc_table_sim_particular_nb(nb_name)
