...
def compute_table_size(table):
    ...
    """
    storage_cost = 0
    for rindex, row in table.count().iteritems(): #.iteritems()は, DataFrameの(column name, Series)を返す．
        attr = table[rindex].dtype 
        if attr == int: # int: 4byte
            storage_cost = storage_cost + float(row * 4)
        elif attr == float: # float: 8byte
            storage_cost = storage_cost + float(row * 8)
        elif attr == str: #文字列長だけ追加
